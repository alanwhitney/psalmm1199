...
import { BookOpen, Bookmark, StickyNote, Trash2, LogOut, ChevronRight, ArrowLeft } from "lucide-react";
...
export default function BookmarksClient({ bookmarks: initial, notes, userEmail }: Props) {
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.textPrimary }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 24px" }}>
        ...
        <p style={{ fontSize: 13, color: C.textMuted, margin: "0 0 32px" }}>
          {bookmarks.length} bookmark{bookmarks.length !== 1 ? "s" : ""} · {notes.length} note{notes.length !== 1 ? "s" : ""}
        </p>

        {/* Tabs */}
        <div style={{ display: "flex", gap: 4, marginBottom: 24, borderBottom: `1px solid ${C.border}`, paddingBottom: 0 }}>
          {([["bookmarks", "Bookmarks", bookmarks.length], ["notes", "Notes", notes.length]] as const).map(([key, label, count]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              style={{
            ...
            >
              {label} <span style={{ fontSize: 11, fontWeight: 400, marginLeft: 4 }}>({count})</span>
            </button>
          ))}
        </div>
        ...
        {tab === "bookmarks" && (
          <div>
            {bookmarks.length === 0 ? (
              <EmptyState
                icon={<Bookmark size={28} color={C.textMuted} />}
                title="No bookmarks yet"
                message="While reading, tap the Bookmark button to save your place in any chapter."
                action={{ href: "/bible/GEN/1", label: "Start reading" }}
              />
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {bookmarks.map((bm) => (
                  <BookmarkCard key={bm.id} bookmark={bm} onDelete={() => deleteBookmark(bm.id)} />
                ))}
              </div>
            )}
          </div>
        ...
        {tab === "notes" && (
          <div>
            {notes.length === 0 ? (
              <EmptyState
                icon={<StickyNote size={28} color={C.textMuted} />}
                title="No notes yet"
                message="While reading, tap Add note to write thoughts on any chapter."
                action={{ href: "/bible/PSA/119", label: "Start reading" }}
              />
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {notes.map((note) => (
  ...
  return (
    <div style={{ background: C.bgRaised, border: `1px solid ${C.border}`, borderRadius: 10, padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        <Link
          href={`/bible/${bookmark.book_id}/${bookmark.chapter}?t=${bookmark.translation}`}
          style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: C.textSecondary, textDecoration: "none", padding: "6px 10px", background: C.bgOverlay, borderRadius: 6, border: `1px solid ${C.border}` }}
        >
        ...
        {confirming ? (
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={onDelete} style={{ fontSize: 11, padding: "5px 10px", background: "#ef444420", border: "1px solid #ef444440", borderRadius: 6, color: "#f87171", cursor: "pointer" }}>
              Remove
            </button>
            <button onClick={() => setConfirming(false)} style={{ fontSize: 11, padding: "5px 10px", background: C.bgOverlay, border: `1px solid ${C.border}`, borderRadius: 6, color: C.textMuted, cursor: "pointer" }}>
              Cancel
            </button>
          </div>
        ) : (
...
function NoteCard({ note }: { note: Props["notes"][0] }) {
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 13, color: C.textSecondary, margin: 0, lineHeight: 1.6, fontStyle: "italic" }}>
        "{preview}"
      </p>
...
function EmptyState({ icon, title, message, action }: {
  icon: React.ReactNode; title: string; message: string; action: { href: string; label: string };
}) {
  return (
